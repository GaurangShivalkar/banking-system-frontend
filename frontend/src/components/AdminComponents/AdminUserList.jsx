    ...
    return (
      <div className="container mx-auto p-4">
        ...
        {filteredList.length > 0 ? (
          <table id="admin-users-table" className="min-w-full bg-white border-collapse shadow-lg">
            ...
            <tbody>
              {filteredList.map((user, index) => (
                <tr key={index} className="text-center odd:bg-gray-100 even:bg-gray-200">
                  ...
                  <td className="py-2 px-4 border">{user.customer?.customerId ?? 'N/A'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <button onClick={() => generatePdf("admin-users-table","admin-users-pdf")} className="w-full bg-blue-600 text-white p-2 rounded mt-4 hover:bg-gray-800">Download PDF</button>
      </div>
    );
  };
